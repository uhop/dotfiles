...
# Claude Code share the same skill tree (chezmoi-managed via
...
#                                                     into the chezmoi source (option (c)
#                                                     from the design discussion: never
#                                                     clobber unmirrored windsurf content)
...
# Claude side must exist (chezmoi just deployed it).
if [ ! -d "$claude_skills" ]; then
  echo "skills bridge: $claude_skills not found, skipping"
  exit 0
fi
...
if [ ${#unmirrored[@]} -gt 0 ]; then
  ...
  echo "  To migrate them: copy each listed skill into the chezmoi source at" >&2
  echo "    \$(chezmoi source-path)/private_dot_claude/skills/<name>/" >&2
  echo "  commit + push, then re-run 'chezmoi update' on this machine." >&2
  echo "  This script will then replace $windsurf_skills with a symlink." >&2
  exit 1
fi
