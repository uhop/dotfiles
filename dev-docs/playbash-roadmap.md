...
- ~~**Inventory schema bump.**~~ *Resolved in milestone 14.* The `managed: false` marker uses the existing extra-fields passthrough in the inventory loader. No schema change needed; default is probe-based detection.
- **Staging cleanup lifecycle.** When does a stale staging dir on a remote get removed? Staging uses `~/.cache/playbash-staging/` (survives reboots, no tmpwatch concern). Three options: never (harmless — one small file), explicit `playbash clean <host>`, or automatic on next contact when the local cache invalidates. Decide during milestone 15 or 16.
