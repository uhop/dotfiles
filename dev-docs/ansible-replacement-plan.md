...
- **Bash completions.** Subcommands (`run`, `debug`, `list`, `hosts`, `log`), playbook names (glob `~/.local/bin/playbash-*`), host names (read from `~/.config/playbash/inventory.json`), `--self`/`-n`/`-p`. The other CLIs in this repo expose a `--bash-completion` flag that prints a completion script — playbash should do the same so installation is `playbash --bash-completion >> ~/.bash_completion` or similar. Small enough to fold into any v2 milestone, but tracked here so it's not forgotten.
- **Offline host detection.** A managed host can be offline (powered down, network split, ssh listener restarted, etc.). Today the runner just lets ssh time out, which can take 30+ seconds per host and clutters the fan-out. Detect upfront and skip with a clear status. Two reasonable approaches:
  - **TCP connect to port 22** with a short timeout (1–2s). Tests the actual path we're about to use. Doesn't require ICMP. Standard pattern.
  - **`ssh -o ConnectTimeout=2 -o BatchMode=yes <host> true`**. Slightly heavier (full ssh handshake) but uses the user's ssh config (jump hosts, port, key, etc.) automatically. Probably the right choice for parity with the actual run path.
  - Either way, surface offline hosts as a distinct status in the StatusBoard and the post-run summary (e.g. `· web2 offline`), and exclude them from the success/failure counts. Probably also worth a `--no-precheck` escape hatch for rare cases where the precheck itself is too slow or unreliable.
