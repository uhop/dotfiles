...
It all started as a gist to keep track of my dotfiles:
https://gist.github.com/uhop/f11632fa81bff6fa4c25300656dce6e7

Now I decided to use [chezmoi](https://www.chezmoi.io/) to manage them across
computers.
...
- Initialize dotfiles:
  ...
  -`` The ini
   ...
   tial script installs various utilities using `apt`. Thus it requires a `sudo` password. Don't be alarmed.
...
- Run:
  ...
  fc-cache -fv
  ```
FiraCode Nerd Font is used for `code` (Visual Studio Code

. It supports common programming ligatures.
...
-o support cut-and-paste in the micro editor:
-
- -se the default (`"external"`) for the clipboard option.
...
See: https://support.google.com/titansecuritykey/answer/9148044?hl=enA
key should be registered o nly on

e. When I attempted to do so, I was misleadingly informed that
...
-hey are installed manually using Win-A (Cmd-A) from Gnome.
-
- -flatpak`
  - `flatseal` &mdash; `flatpak` permission editor.
  - -calibre` &mdash; an e-book manager.
  - -eb browsers (used for testing):
    - Brave
    - Chromium
  - - Google Chrome
    - Microsoft Edge
...
- - `steam` &mdash; the game launcher from Valve.
  - `wezterm` &mdash; a modern termina.
- -snap`
...
- - `postman` &mdash; a tool for debugging network services.
  - - It is available as a flatpak, but apparently it is compl
   etely unusable.
- `AppImageLauncher`
