...
6. **Output polish.** ✅ Per-host summary uses minimal color (green ✓, bold-white-on-red ✗ block, magenta ⏵ for actions, orange ⚠ for warnings — orange chosen for dark/light terminal compatibility, replacing yellow), no `playbash:` prefix lines, no section headers (each event has its own glyph), log path printed only on failure. `NO_COLOR` and `PLAYBASH_NO_COLOR` env overrides honored.
...
### v3 — portability to vanilla hosts

Goal: run playbash against any Linux/Mac host with `bash`, `ssh`, and `python3` (≥ 3.9) without requiring chezmoi or any pre-deployment. Heterogeneous fleets (mix of chezmoi-managed and vanilla hosts) are first-class. Builds on the two Mac PTY fixes from milestone 10 — those were prerequisites the wrapper needed before it was safe to deploy across arbitrary hosts via an upload step.

**ssh authentication requirement.** All ssh invocations from the runner are non-interactive: the spawn uses `stdio: ['ignore', 'pipe', 'pipe']` AND `detached: true` (which calls `setsid(2)`, leaving the child without a controlling terminal). ssh therefore has nowhere to prompt for a key passphrase or remote password — `/dev/tty` is unopenable and `SSH_ASKPASS` is almost always unset. **Passwordless ssh is a hard requirement** (same as Ansible / pyinfra / fabric): the operator must have either ssh-agent running with the key loaded, or public-key auth pre-configured per host. As of v3, every ssh spawn passes `-o BatchMode=yes` so that any auth that would require interaction fails immediately with a clear error message instead of dancing through the `/dev/tty` / `SSH_ASKPASS` fallback path and producing a confusing "permission denied" several seconds later. Bootstrapping a vanilla host's `~/.ssh/authorized_keys` is an interactive setup step done outside the runner — see [Future § `playbash bootstrap`](#future-post-v3--discussion-needed).
...
14. **Wrapper staging primitive.** Internal helper that ensures the python wrapper is present on a remote host before a run. For chezmoi-managed hosts, no-op — the wrapper is already at `~/.local/libs/playbash-wrap.py`. For vanilla hosts, `mktemp -d` once per host, push the wrapper (probably via `cat <wrapper> | ssh host 'cat > <staging>/playbash-wrap.py'` since the wrapper is small and this avoids depending on `scp`/`rsync`), return the staging dir path. Result cached locally under `~/.cache/playbash/staging/<hostkey>/` keyed by the wrapper's sha so a `chezmoi apply` that updates the wrapper naturally invalidates all per-host staging dirs. Cleanup via cache eviction or an explicit `playbash clean <host>` subcommand. No user-visible CLI surface for the primitive itself — it's the substrate consumed by milestones 15, 16, and 17.
    ...
    **`BatchMode=yes` rolled in.** Every ssh invocation introduced by this milestone — *and* the existing `runRemote()` / `runFanout()` ssh spawns, updated in the same patch — passes `-o BatchMode=yes`. Auth failures become fast and deterministic instead of confusing: ssh exits immediately with `Permission denied (publickey,password)` rather than failing to open `/dev/tty` after a delay. One-line argv addition per `spawn()` call site, big DX win. Could be cherry-picked as a standalone v2 fix any time before v3 starts; will definitely land as part of this milestone otherwise.

15. **`playbash exec <host> <command...>`.** One-shot command execution through the same wrapper / sidecar / rectangle pipeline as `playbash run`, without a playbook script. The wrapper invokes `bash -c '<command>'` instead of `exec <script>`. No `playbash.sh` dependency on the remote (no helper calls), so the only thing staged is the wrapper itself (via milestone 14). Same fan-out shape as `run`: groups, parallel, per-host status board, per-host log file. Sidecar is empty for exec (or could be repurposed for a stdout/stderr split — decide during implementation). Useful for "run this on every server right now" without authoring a playbook script first.

16. **Upload mode for `playbash run`.** Extends milestone 14 to also stage `playbash.sh` and the resolved playbook script (`~/.local/bin/playbash-<name>` on the operator) into the same scratch dir. Sets `PLAYBASH_LIBS=<scratch>` so the playbook's `source` directive picks up the staged copy (relies on milestone 13). New `runRemoteUpload()` mirrors `runRemote()`; `dispatchRun()` picks between them based on per-host capability detection from milestone 14, transparently from the user's point of view. Same per-host caching and invalidation as the wrapper.
...
17. **`playbash put` / `playbash get`.** User-facing file staging primitives built on the same transfer machinery as milestone 14. `playbash put <local-path> <hosts>:<remote-path>` and `playbash get <hosts>:<remote-path> <local-path>`. Same fan-out, parallel, group expansion, and per-host status board as `run`. Useful for "ship this config file everywhere" or "fetch yesterday's log from every host" workflows. Implementation note: mostly a thin wrapper around `scp` (or `rsync` when available, falling back to `scp`) with the playbash UX on top. No interaction with the python wrapper or sidecar.

18. **Offline host detection.** Pre-flight check before any ssh work, so an offline host fails fast with a distinct status instead of clogging the fan-out for 30+ seconds while ssh times out. Implementation: `ssh -o ConnectTimeout=2 -o BatchMode=yes <host> true` once per target at the start of the run, parallelized across the target list. Uses the actual ssh path so jump hosts, port overrides, and key selection from `~/.ssh/config` apply for free; lighter false-positive rate than a raw TCP connect to port 22 since it exercises the full handshake. Surface offline hosts as a distinct status in the StatusBoard (e.g. `· web2 offline`) and the post-run summary; exclude them from the success/failure counts. Add a `--no-precheck` escape hatch for the rare case where the precheck itself is too slow or unreliable. Independent of the rest of v3 — could land before milestone 14 if convenient.

19. **Bash completions.** Subcommands (`run`, `debug`, `list`, `hosts`, `log`, plus the new v3 ones: `exec`, `put`, `get`, `clean`), playbook names (glob `~/.local/bin/playbash-*`), host names and group names (read from `~/.config/playbash/inventory.json`), `--self` / `-n` / `-p` / `--no-precheck`. Match the `--bash-completion` convention used by other CLIs in this repo: `playbash --bash-completion` prints a completion script to stdout, install via `playbash --bash-completion >> ~/.bash_completion` or whichever mechanism the user prefers. Big DX win — host name and playbook name completion alone makes the tool dramatically faster to drive interactively. Independent of the portability work — slot in whenever convenient.

**Open questions for v3** (decide during implementation, not before):
...
- **Mac wrapper deployment.** Vanilla Mac hosts hit the same milestone-10 PTY bugs the chezmoi wrapper just fixed. Milestone 14's staging primitive must upload **the canonical wrapper from this repo**, not a vendored or simplified copy — single source of truth. The wrapper file is small enough that there's no temptation to inline-copy it elsewhere; just resist if the temptation arises.
...
### Future (post-v3 — discussion needed)
...
- **Node entrypoint shape:** ✅ split landed in milestone 8. Layout (refined post-milestone-8 to move modules out of `bin/`):
  - `private_dot_local/bin/executable_playbash` — entry, dispatcher, run mechanics, list/hosts/log commands.
  - `private_dot_local/share/playbash/render.js` — `COLOR`, `Rectangle`, `HostSlot`, `StatusBoard`, sanitizer, truncator, status line builder.
  - `private_dot_local/share/playbash/inventory.js` — load, resolve hosts/groups/`all`, self detection, target filtering.
  - `private_dot_local/share/playbash/sidecar.js` — JSON-lines parser, per-host summary renderer, cross-host aggregator.
  - General-purpose helpers (`comp.js`, `semver.js`, `nvm.js`) live in `private_dot_local/share/utils/` and are imported by `executable_trim-node-versions.js` and `executable_update-node-versions.js`.
...
- **Wiki sync after v2 closes** (mechanical, not a design decision — tracked here so it isn't forgotten). When milestones 11 and 12 land, update/verify the four `external_wiki/` files that mention playbash:
  - `Playbash-Server-Management.md` — primary doc; will need the most edits (milestone-10 sudo detection + the two Mac PTY fixes from this session, then milestone-11 macOS-target support and milestone-12 `upd --restart-services`).
  - `Utilities.md` — likely a one-line entry; spot-check it still matches reality.
  ...
  Also re-run the same audit when v3 ships, since vanilla-host workflow + `playbash exec`/`put`/`get`/`bootstrap` will reshape the user-facing surface significantly.
