...
│   └── detect-distro.sh               # bootstrap detection library (identity + version utils)
...
- Bootstrap detection library lives at `.chezmoitemplates/detect-distro.sh` (sourced into `bootstrap-dotfiles`, `.chezmoi.toml.tmpl`, and `run_onchange_*` via `includeTemplate` or inline `{{ template }}`). Currently covers identity probes (`detect::identity`, `family_contains`, `is_version_at_least`) and version utilities (`_version_normalize`, `_version_compare`). Capabilities, resolver, and candidate tables land in follow-up commits within PR 1. Sourcing is idempotent (guarded by `__DETECT_DISTRO_LOADED`). Tests at `tests/detect/` — run with `bash tests/detect/run-tests.sh`. Full design in the bootstrap detection design doc (vault).
